...
def plot_perf(input_sizes: list):
    gold_standard_times = []
    ...
    plt.plot(input_sizes, multiprocess_quicksort_times,
             label='Multiprocess Quicksort')
...
def test_single_perf(input_size):
    ...
    for sort_func in [sorted, quick_sort, quick_sort_th, quick_sort_mp]:
        print(f'{sort_func.__name__}: {test_sort(sort_func, input_size)}s')
